...
```
main            ← 本番（v1.1 が最後の稼働バージョン。v2.0 完成時にここへマージ）
 └─ ver2.0      ← 再設計の統合先（長期ブランチ）
      ├─ feat/xxx
      ├─ fix/xxx
      ├─ chore/xxx
      ├─ test/xxx
...
- v2.0 完成時に `ver2.0 → main` を PR 経由でマージ
...
1. `ver2.0` から `feat/xxx` を切る（例: `feat/domain-types`, `feat/muscle-diagram`, `fix/slider-initial-value`）
2. 実装 → push → GitHub で **`feat/xxx → ver2.0`** の PR を作成
...
```bash
git branch --merged ver2.0 | grep -E '^\s+(feat|fix|chore|test|docs)/' | xargs git branch -d
```
