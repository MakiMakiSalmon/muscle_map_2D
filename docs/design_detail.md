...
> **【凍結・歴史的記録】** 本書は v2.0 の詳細設計。v3.0 の生きた仕様は [docs/v3/design.md](./v3/design.md)（本書との差分 D1〜D8）を参照。本書は v3.0 で上書きされていない箇所のみ有効。
...
// §19-2 の lazy singleton (adminDb) を import する
...
// 遅延初期化（lazy singleton）。
// Next.js の next build はルートモジュールを import するが env vars は未注入のため、
// モジュールロード時に initializeApp を呼ぶとビルドがクラッシュする。
// 関数経由にすることでリクエスト時まで初期化を遅延させ、
// Vercel warm instance での再利用（singleton）も getApps() チェックで維持する。
...
**Route Handler 側の使い方（import して関数として呼ぶ）:**
```typescript
// src/app/api/fatigue/current/route.ts
...
export const GET = withAuth(async (_req, { uid }) => {
  // adminDb() を呼び出して Firestore インスタンスを取得。initializeApp は呼ばない
  const db = adminDb();
  ...
});
```

**§15 との整合:** `verifyUser` は §19-2 の `adminAuth()` を呼び出して利用する形で統一する。`import { adminAuth } from '@/lib/firebase/admin'` + `adminAuth().verifyIdToken(...)` のパターン。Route Handler から直接 `initializeApp` を呼ぶパターンは使わない。
